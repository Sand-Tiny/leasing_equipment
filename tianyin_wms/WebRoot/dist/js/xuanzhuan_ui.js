...
ImageTrans.modes = function () {
    ...
    function initImg(img, container) {
        ...
        $$D.setStyle(img, {
            position: "absolute",
            border: 0, padding: 0, margin:"0 auto", width: "auto", height: "auto",//������ʽ
            visibility: "hidden"//����ǰ����
        });
        _css=$(container).find('img').attr('class')
        _style=$(container).find('img').attr('style')
           $(container).find('img').remove();
        
        container.appendChild(img);
        _style1=$(img).attr('style')
        img.className=_css
        $(img).attr('style',_style+_style1)
    }
    //��ȡ�任��������
